...
$lode="ACDEFGHIKLMNPQRSTVWYCGTA";
 ...
 for $k (0..$#out )
 {
   ...
   for $i (0..$#prot)
   {
     ...
     if (($prot[$i]->{atom} eq "CA" || $prot[$i]->{atom} eq "P")&& $prot[$i]->{chain} eq $sequences[$k]){
...
format PDB_FORMAT=
@<<<<<<@>>>  @<<<@<< @<@>>>   @>>>.@<<@>>>.@<<@>>>.@<<@>>.@<@>>.@<
$typew,$atnumw,$atomw,$resw,$chainw,$resnumw,$x1,$x2,$y1,$y2,$z1,$z2,$o1,$o2,$b1,$b2
.
